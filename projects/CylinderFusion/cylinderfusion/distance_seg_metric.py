...
class DistanceSegMetric(BaseMetric):
    ...
    def __init__(self,
        ...
        super(DistanceSegMetric, self).__init__(
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        Args:
            ...
            data_samples (Sequence[dict]): A batch of outputs from
                the model.
        """
        for data_sample, points in zip(data_samples, data_batch['inputs']['points']):
    ...
    def format_results(self, results):
        ...
        for i, (eval_ann, result, points) in enumerate(results):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        distances = [0, 10, 20, 30, 50, 100, 200]
        ...
        for i in range(len(distances) - 1):
            ...
            for eval_ann, sinlge_pred_results, points in results:
                ...
                idx = np.where((dist > distances[i]) & (dist < distances[i+1]))
                gt_semantic_masks.append(eval_ann['pts_semantic_mask'][idx])
                pred_semantic_masks.append(
                    sinlge_pred_results['pts_semantic_mask'][idx])
                
            print_log(f'compute {distances[i]}-{distances[i+1]} metrics--------------', logger=logger)
        ...
        for eval_ann, sinlge_pred_results, points in results:
            dist = np.linalg.norm(points[:, :3], axis=1)
            idx = np.where((dist > distances[0]) & (dist < distances[-1]))
            gt_semantic_masks.append(eval_ann['pts_semantic_mask'][idx])
            pred_semantic_masks.append(
                sinlge_pred_results['pts_semantic_mask'][idx])
            
        print_log(f'compute {distances[0]}-{distances[-1]} metrics--------------', logger=logger)
