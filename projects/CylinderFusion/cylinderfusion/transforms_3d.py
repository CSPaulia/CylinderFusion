...
@TRANSFORMS.register_module()
class ImageAndAnnotationAug3D(ImageAug3D):

    def anno_transform(self, img, resize_dims, crop, flip, rotate):
        # adjust image
        img = Image.fromarray(img.astype('uint8'), mode='L')
        img = img.resize(resize_dims, resample=Resampling.NEAREST)
        img = img.crop(crop)
        if flip:
            img = img.transpose(method=Image.FLIP_LEFT_RIGHT)
        img = img.rotate(rotate)
    ...
    def transform(self, data: Dict[str, Any]) -> Dict[str, Any]:
        imgs = data['img']
        img_annotations = data['gt_semantic_mask_2d']
        new_imgs = []
        new_img_annotations = []
        transforms = []
        for img, img_annotation in zip(imgs, img_annotations):
            resize, resize_dims, crop, flip, rotate = self.sample_augmentation(
                data)
            post_rot = torch.eye(2)
            post_tran = torch.zeros(2)
            ...
            new_img_annotation = self.anno_transform(
                img_annotation,
                resize_dims=resize_dims,
                crop=crop,
                flip=flip,
            ...
            transform[:2, 3] = translation
            new_imgs.append(np.array(new_img).astype(np.float32))
            new_img_annotations.append(np.array(new_img_annotation).astype(np.int8))
            transforms.append(transform.numpy())
        data['img'] = new_imgs
        data['gt_semantic_mask_2d'] = new_img_annotations
        # update the calibration matrices
        data['img_aug_matrix'] = transforms
        return data
...
@TRANSFORMS.register_module()
class BEVFusionRandomHorizontalFlip3D:
    """Compared with `RandomFlip3D`, this class directly records the lidar
    augmentation matrix in the `data`."""
...
@TRANSFORMS.register_module()
class BEVFusionRandomVerticalFlip3D:
    """Compared with `RandomFlip3D`, this class directly records the lidar
    augmentation matrix in the `data`."""
