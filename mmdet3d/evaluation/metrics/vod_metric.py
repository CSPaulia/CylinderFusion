...
class VoDMetric(BaseMetric):
    ...
    def __init__(self,
        ...
        self.default_prefix = 'VoD metric'
    ...
    def convert_annos_to_vod_annos(self, data_infos: dict) -> List[dict]:
        """Convert loading annotations to VoD annotations.
        ...
        Returns:
            List[dict]: List of VoD annotations.
        """
            ...
            for i, annos in enumerate(data_annos):
                if len(annos['instances']) == 0:
                    ...
                    for instance in annos['instances']:
                        ...
                        vod_annos['name'].append(label2cat[label])
                        vod_annos['truncated'].append(instance['truncated'])
                        vod_annos['occluded'].append(instance['occluded'])
                        vod_annos['alpha'].append(instance['alpha'])
                        vod_annos['bbox'].append(instance['bbox'])
                        vod_annos['location'].append(instance['bbox_3d'][:3])
                        vod_annos['dimensions'].append(
                            instance['bbox_3d'][3:6])
                        vod_annos['rotation_y'].append(
                            instance['bbox_3d'][6])
                        vod_annos['score'].append(instance['score'])
                    for name in vod_annos:
                        vod_annos[name] = np.array(vod_annos[name])
                data_annos[i]['vod_annos'] = vod_annos
        return data_annos
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            result['origin_idx'] = data_sample['origin_idx']
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        self.data_infos = self.convert_annos_to_vod_annos(pkl_infos)
        ...
        gt_annos = [
            self.data_infos[result['sample_idx']]['vod_annos']
            for result in results
        ]

        for metric in self.metrics:
            ap_dict = self.vod_evaluate(
                result_dict,
                gt_annos,
                metric=metric,
                logger=logger,
    ...
    def format_results(
        ...
        origin_idx_list = [result['origin_idx'] for result in results]
        for name in results[0]:
            ...
            if 'pred_instances' in name and '3d' in name and name[
                ...
                result_list_ = self.bbox2result_vod(net_outputs,
                                                      sample_idx_list, origin_idx_list,
                                                      classes, pklfile_prefix_,
                                                      submission_prefix_)
                result_dict[name] = result_list_
            elif name == 'pred_instances' and name[0] != '_' and results[0][
                    name]:
                net_outputs = [result[name] for result in results]
                result_list_ = self.bbox2result_vod2d(
                    net_outputs, sample_idx_list, origin_idx_list, 
                    classes, pklfile_prefix_, submission_prefix_)
                result_dict[name] = result_list_
    ...
    def bbox2result_vod(
            ...
            origin_idx_list: List[int],
            class_names: List[str],
            pklfile_prefix: Optional[str] = None,
            submission_prefix: Optional[str] = None) -> List[dict]:
        """Convert 3D detection results to vod format for evaluation and test
        submission.
        ...
        Returns:
            List[dict]: A list of dictionaries with the vod format.
        """
        ...
        for idx, pred_dicts in enumerate(
            ...
            origin_idx = origin_idx_list[idx]
            ...
            if submission_prefix is not None:
                curr_file = f'{submission_prefix}/{origin_idx:05d}.txt'
    ...
    def bbox2result_vod2d(
            ...
            pklfile_prefix: Optional[str] = None,
            submission_prefix: Optional[str] = None) -> List[dict]:
        """Convert 2D detection results to vod format for evaluation and test
        submission.
        ...
        if submission_prefix is not None:
            ...
            for i, anno in enumerate(det_annos):
                sample_idx = sample_idx_list[i]
                origin_idx = origin_idx_list[idx]
                cur_det_file = f'{submission_prefix}/{origin_idx:05d}.txt'
