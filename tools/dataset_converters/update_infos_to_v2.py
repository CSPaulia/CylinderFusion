...
def update_nuscenes_infos(pkl_path, out_dir):
    ...
    nusc = NuScenes(
        ...
        dataroot='/data1/csj_data/dataset/nuscenes_full_dataset/',
        verbose=True)
...
def update_vod_infos(pkl_path, out_dir):
    ...
    # TODO discuss how to process 'Van', 'DontCare'
    METAINFO = {
        'classes': ('Car', 'Pedestrian', 'Cyclist', 'rider',
                    'bicycle', 'bicycle_rack', 'human_depiction',
                    'moped_scooter', 'motor', 'ride_other',
                    'ride_uncertain', 'truck', 'vehicle_other'),
    }
    print(f'Reading from input file: {pkl_path}.')
    data_list = mmengine.load(pkl_path)
    print('Start updating:')
