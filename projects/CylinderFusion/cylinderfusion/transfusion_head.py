...
from .gaussian import draw_cylinder_polar_heatmap_gaussian
...
from .transformer import DeformableTransformerDecoder, DeformableTransformer
from .transformer import LearnedPositionalEncoding3D
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x_q, x_kv):
        ...
        x_q = x_q.view(B, C, -1).permute(0, 2, 1)  # (B, HW, C)
        x_kv = x_kv.view(B, C, -1).permute(0, 2, 1)  # (B, HW, C)

        # Linear projections
        Q = self.query_proj(x_q)  # (B, HW, C)
        K = self.key_proj(x_kv)  # (B, HW, C)
        V = self.value_proj(x_kv)  # (B, HW, C)
        ...
        out = torch.matmul(attn, V)  # (B, num_heads, HW, C // num_heads)
...
class CrossAttentionFuser(nn.Module):
    def __init__(self, in_channels: List[int], out_channels: int, num_heads: int = 8):
        super(CrossAttentionFuser, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels

        # Ensure that the dimensions are compatible for cross attention
        assert in_channels[0] == in_channels[1], "Lidar and Camera channels must match."

        self.cross_attention = CrossAttention(in_channels[0], num_heads)
    ...
    def forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        ...
        # Apply cross attention where lidar features act as queries and camera features as keys and values
        fused_features = self.cross_attention(lidar_features, camera_features)

        # You could also do the reverse to fuse the features more strongly
...
@MODELS.register_module()
class DeformableCrossAttentionFuser(nn.Module):
    def __init__(
            self, 
            in_channels: List[int], 
            out_channels: int,
            transformer_config=None):
        super(DeformableCrossAttentionFuser, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels

        assert self.in_channels[0] == self.in_channels[1]

        self.x_ref = torch.tensor(transformer_config.get('n_points', 9)**0.5, dtype=torch.long)
        self.y_ref = torch.tensor(transformer_config.get('n_points', 9)**0.5, dtype=torch.long)

        self.cross_attention = DeformableTransformerDecoder(
            self.in_channels[0],
        ...
        if self.pos_embedding_type == 'linear':
            self.pos_embedding = nn.Linear(2, self.in_channels[0])
        else:
            raise NotImplementedError()

    def create_2D_grid(self, x_size, y_size, device):
        ...
        coord_base = coord_base.view(1, 2, -1).permute(0, 2, 1).to(device)
        return coord_base
    ...
    def forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert lidar_features.shape[2:4] == camera_features.shape[2:4]

        query_feats = lidar_features.view(B, C, -1).permute(0, 2, 1)
        key_feats = camera_features.view(B, C, -1).permute(0, 2, 1)

        pos_features = self.create_2D_grid(H, W, lidar_features.device)
        ...
        # Apply cross attention where lidar features act as queries and camera features as keys and values
        out_dicts = self.cross_attention(
            query_feats,
            self.pos_embedding,
            key_feats,
            spatial_shapes,
        ...
        fused_features = out_dicts['fused_features']
        fused_features = fused_features.permute(0, 2, 1).view(B, C, H, W)

        return fused_features
    

@MODELS.register_module()
class DCALearnedPEFuser(nn.Module):
    def __init__(
            self, 
            img_in_channels: int, 
            ...
            bev_size: List[int],
            transformer_config=None):
        super(DCALearnedPEFuser, self).__init__()
        self.in_channels = img_in_channels
        self.out_channels = out_channels

        if img_in_channels != radar_in_channels:
            self.radar_reduc_conv = ConvModule(
                radar_in_channels,
                img_in_channels,  #rac change imc
                kernel_size=1,
                padding=0,
                conv_cfg=None,
        ...
        self.cross_attention1 = DeformableTransformer(
            self.in_channels,
            n_levels=transformer_config.n_levels,
            n_heads=transformer_config.n_heads,
            dim_single_head=self.in_channels // transformer_config.n_heads,
            dim_ffn=transformer_config.dim_ffn,
            dropout=transformer_config.dropout,
            n_points=transformer_config.get('n_points', 9),
        )

        # self.cross_attention2 = DeformableTransformer(
        #     self.in_channels,
        #     n_levels=transformer_config.n_levels,
        #     n_heads=transformer_config.n_heads,
        #     dim_single_head=self.in_channels // transformer_config.n_heads,
        ...
        self.LearnedPositionalEncoding1=LearnedPositionalEncoding3D(num_feats=self.in_channels // 2, row_num_embed=self.H, col_num_embed=self.W)
        self.LearnedPositionalEncoding2=LearnedPositionalEncoding3D(num_feats=self.in_channels // 2, row_num_embed=self.H, col_num_embed=self.W)

        self.ConvFuser = ConvFuser(
            in_channels = [self.in_channels, self.in_channels],
    ...
    @staticmethod
    def get_reference_points(H, W, device, bs=1, dtype=torch.float):
    ...
    @property
    def with_radar_reduc_conv(self):
        """bool: Whether the fuser has a reduction convolution block.
        """
        return hasattr(self, 'radar_reduc_conv') and self.radar_reduc_conv is not None
    ...
    def forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        ...
        if self.with_radar_reduc_conv:
            lidar_features = self.radar_reduc_conv(lidar_features)
        lidar_feats = rearrange(lidar_features, 'b c h w -> b (h w) c')
        camera_feats = rearrange(camera_features, 'b c h w -> b (h w) c')

        mask = torch.zeros(1, 1, self.H, self.W).to(device)
        lidar_pos = self.LearnedPositionalEncoding1(mask)
        camera_pos = self.LearnedPositionalEncoding2(mask)

        lidar_ref_points = self.get_reference_points(H=self.H, W=self.W, device=device)
        # camera_ref_points = self.get_reference_points(H=self.H, W=self.W, device=device)

        fusion_query_lidar = self.cross_attention1(
            query=lidar_feats,
            query_pos=lidar_pos,
            reference_points=lidar_ref_points,
            key=camera_feats,
            key_pos=camera_pos,
            key_spatial_shapes=torch.tensor([(self.H, self.W)]).to(device),
            level_start_index=torch.tensor([0, self.H*self.W]).to(device)
        )
        # fusion_query_camera = self.cross_attention2(
        #     query=camera_feats,
        #     query_pos=camera_pos,
        #     reference_points=camera_ref_points,
        #     key=lidar_feats,
        #     key_pos=lidar_pos,
        #     key_spatial_shapes=torch.tensor([(self.H, self.W)]).to(device),
        #     level_start_index=torch.tensor([0, self.H*self.W]).to(device)
        # )
        fusion_query_lidar = rearrange(fusion_query_lidar, 
                                       'b (h w) c -> b c h w', 
                                       h=self.H, 
                                       w=self.W) 
        # fusion_query_camera = rearrange(fusion_query_camera, 
        #                                 'b (h w) c -> b c h w', 
        #                                 h=self.H, 
        #                                 w=self.W)

        # fusion_feats = self.ConvFuser([fusion_query_lidar, fusion_query_camera])
        fusion_feats = self.ConvFuser([fusion_query_lidar, lidar_features])

        return fusion_feats
    

@MODELS.register_module()
class FullyDCALearnedPEFuser(nn.Module):
    def __init__(
            self, 
            img_in_channels: int, 
            radar_in_channels: int, 
            ...
            bev_size: List[int],
            transformer_config=None):
        super(FullyDCALearnedPEFuser, self).__init__()
        self.in_channels = img_in_channels
        self.out_channels = out_channels

        assert img_in_channels == radar_in_channels
    ...
    def forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        camera_features, lidar_features = inputs
        device = lidar_features.device
        assert lidar_features.shape[1:4] == camera_features.shape[1:4]
        ...
        return fusion_feats.contiguous()
...
class TransFusionHead(nn.Module):
    ...
    def forward_single(self, inputs, metas):
        ...
        # elif self.test_cfg[
        #         'dataset'] == 'VoD':  # for Pedestrian & Cyclist in VoD
...
@MODELS.register_module()
class CylinderTransFusionHead(TransFusionHead):
    
    def recover_bbox(self, bboxes):
        # polar --> （x,y）
        # rho = bboxes.tensor[:, 0].clone()
        # phi = bboxes.tensor[:, 1].clone()
        # bboxes.tensor[:, 0] = rho * torch.cos(phi)
        # bboxes.tensor[:, 1] = rho * torch.sin(phi)
        rho = bboxes[:, 0].clone()
        phi = bboxes[:, 1].clone()
        bboxes[:, 0] = rho * torch.cos(phi)
        bboxes[:, 1] = rho * torch.sin(phi)
        bboxes[:, 4] = rho * bboxes[:, 4]
    ...
    def predict_by_feat(self,
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            for i in range(batch_size):
                boxes3d = temp[i]['bboxes']
                # xy --> polar
                boxes3d = self.recover_bbox(boxes3d)
                scores = temp[i]['scores']
                labels = temp[i]['labels']
                # adopt circle nms for different categories
                if self.test_cfg['nms_type'] is not None:
    ...
    def get_targets_single(self, gt_instances_3d, preds_dict, batch_idx):
        ...
        for idx_layer in range(num_layer):
            ...
            # recover xyz from polar coordinate
            # print('bbox coder rho result:', bboxes_tensor_layer[0, 0], 'bbox coder phi result:', bboxes_tensor_layer[0, 1])
            rho = bboxes_tensor_layer[:, 0].clone()
            phi = bboxes_tensor_layer[:, 1].clone()
            bboxes_tensor_layer[:, 0] = rho * torch.cos(phi)
            bboxes_tensor_layer[:, 1] = rho * torch.sin(phi)
            bboxes_tensor_layer[:, 4] = rho * bboxes_tensor_layer[:, 4]
            # print('bbox coder x result:', bboxes_tensor_layer[0, 0], 'bbox coder y result:', bboxes_tensor_layer[0, 1])
        ...
        if len(pos_inds) > 0:
            ...
            # print('pos_gt_bboxes x:', pos_gt_bboxes[0, 0], 'pos_gt_bboxes y:', pos_gt_bboxes[0, 1])
            rho = torch.sqrt(pos_gt_bboxes[:, 0]**2 + pos_gt_bboxes[:, 1]**2)
            phi = torch.atan2(pos_gt_bboxes[:, 1], pos_gt_bboxes[:, 0])
            pos_gt_bboxes[:, 0] = rho
            pos_gt_bboxes[:, 1] = phi
            pos_gt_bboxes[:, 4] = pos_gt_bboxes[:, 4] / rho
            # print('pos_gt_bboxes rho:', pos_gt_bboxes[0, 0], 'pos_gt_bboxes phi:', pos_gt_bboxes[0, 1])
        ...
        for idx in range(len(gt_bboxes_3d)):
            ...
            if width > 0 and length > 0:
                ...
                # xy --> polar
                # print('gt_bboxes_3d x:', x, 'gt_bboxes_3d y:', y)
                rho = torch.sqrt(x**2 + y**2)
                phi = torch.atan2(y, x)
                x = rho
                y = phi
                # print('gt_bboxes_3d rho:', x, 'gt_bboxes_3d phi:', y)
                ...
                # draw_heatmap_gaussian(heatmap[gt_labels_3d[idx]], center_int, radius) # noqa: E501
                # NOTE: fix
                draw_cylinder_polar_heatmap_gaussian(heatmap[gt_labels_3d[idx]],
                                                     center_int, radius,
