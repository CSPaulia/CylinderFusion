...
class Pack3DDetInputs(BaseTransform):
    ...
    def pack_single_results(self, results: dict) -> dict:
        ...
        if 'gt_semantic_mask_2d' in results:
            if isinstance(results['gt_semantic_mask_2d'], list):
                # process multiple imgs in single frame
                img_annos = np.stack(results['gt_semantic_mask_2d'], axis=0)
                if img_annos.flags.c_contiguous:
                    img_annos = to_tensor(img_annos).contiguous()
                else:
                    img_annos = to_tensor(
                        np.ascontiguousarray(img_annos))
                results['gt_semantic_mask_2d'] = img_annos
            else:
                img_annos = results['gt_semantic_mask_2d']
                if len(img_annos.shape) < 3:
                    img_annos = np.expand_dims(img_annos, -1)
                # To improve the computational speed by by 3-5 times, apply:
                # `torch.permute()` rather than `np.transpose()`.
                # Refer to https://github.com/open-mmlab/mmdetection/pull/9533
                # for more details
                if img_annos.flags.c_contiguous:
                    img_annos = to_tensor(img_annos).permute(2, 0, 1).contiguous()
                else:
                    img_annos = to_tensor(
                        np.ascontiguousarray(img_annos.transpose(2, 0, 1)))
                results['gt_semantic_mask_2d'] = img_annos
